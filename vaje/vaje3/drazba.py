...
for predmet, ime, cena in zapisnik:
    ...
    if ime not in nakupi:
        #nakupi.setdefault(ime, [])
        nakupi[ime] = [predmet]
    elif predmet not in nakupi[ime]:
...
prejsnji_predmet = ""
prejsnja_oseba = ""
osebe_visale_ponudbo = {}

for predmet, ime, cena in zapisnik:
    if prejsnji_predmet == "" or prejsnji_predmet != predmet or prejsnja_oseba == "":
        prejsnji_predmet = predmet
        prejsnja_oseba = ime
        #continue
    else:


        if prejsnji_predmet == predmet:
            if ime not in osebe_visale_ponudbo:
                osebe_visale_ponudbo = ime
                osebe_visale_ponudbo[ime] = 1
            else:
                osebe_visale_ponudbo[ime] += 1

for ime, valuje in osebe_visale_ponudbo:
    print(ime, valuje)